...
/**
 * Queues a job to synchronize employees data
 */
exports.syncFuncionarios = async (req, res, next) => {
  try {
    ...
    res.status(202).json({
      success: true,
      message: 'Employee synchronization job added to queue',
      jobId
    });
  } catch (error) {
    console.error('Error queuing funcionario sync job:', error);
    res.status(500).json({
      success: false,
 ...
 */
exports.syncFuncionariosOriginal = async (req, res, next) => {
  try {
    const userId = req.user.id;
    const { empresaId } = req.query;
    ...
    try {
      ...
      for (const empresa of empresas) {
        ...
        // Processar os dados de funcionários em lotes
        const batchSize = 50;
